...
module internal Emit =
    ...
    let moduleBuilder = lazy (
        ...
        let assemblyBuilder =
            AppDomain.CurrentDomain.DefineDynamicAssembly(AssemblyName(name),AssemblyBuilderAccess.RunAndSave)
        /// Builder for module
        assemblyBuilder, assemblyBuilder.DefineDynamicModule(name+".dll", "Foq.Dynamic.dll", true)
        )
    ...
    let defineType (abstractType:Type) (others:Type list) =
        ...
        (snd moduleBuilder.Value).DefineType(mockName, attributes, parent, interfaces)
    ...
    let mock (mode, abstractType:Type, otherTypes:Type list, 
        ...
        (fst moduleBuilder.Value).Save("Foq.Dynamic.dll") |> ignore
